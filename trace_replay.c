...
/*From here CHOI modified for FTL + trace replay*/
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
#include <stdint.h>
...
int trace_io_get(double* arrival_time, int* devno, int* blkno, int*bcount, int* flags, struct trace_info_t *trace, struct io_stat_t *io_stat){
	...
	if(res!=-1 && trace->trace_io_issue_count < trace->trace_io_cnt){
...
int make_jobs(struct thread_info_t *t_info, struct iocb **ioq, struct io_job **jobq, int depth, long tid){
	...
	//FILE *fp_trace;
	//fp_trace = fopen("trace_HILS.txt", "a");
    /*====================*/
	...
	for(i = 0;i < depth;i++){
		...
		if(trace->trace_io_issue_count >= trace->trace_io_cnt){
			//printf("io over ! \n");
			//req_list->trace_done = 1;
			return cnt;
		}
...
out:
		...
		job->buf = (char*)allocate_aligned_buffer(job->bytes);
		...
		buffer = (unsigned char *)alloc_buffer(job->bytes);
		g_assert(buffer != NULL);

		switch (job->rw) {
		  case 0:
			 gettimeofday(&start, NULL);
			 ret = flash->f_op->write(flash, buffer, job->bytes, job->offset);
			 gettimeofday(&end, NULL);
			 break;
		  case 1:
			 gettimeofday(&start, NULL);
			 ret = flash->f_op->read(flash, buffer, job->bytes, job->offset);
			 gettimeofday(&end, NULL);
			 break;
		}


		g_assert(ret == (ssize_t)job->bytes);
		interval = time_since(&start, &end);
		total_interval += interval;	
		free_buffer(buffer);
		
		th_info[tid].th_jobs[cnt] = job;
		cnt++;
	}
...
void wait_completion(struct thread_info_t *t_info, int cnt, long tid){
	struct io_job *job;
	...
	while(1){
		...
		for(i = 0;i < complete_count;i++){
			// Modified by CHOI  //job = (struct io_job *)((unsigned long)t_info->events[i].obj);
			job = th_info[tid].th_jobs[i];
...
void *sub_worker(void *threadid)
{
	...
	while(trace->trace_io_issue_count < trace->trace_io_cnt){
		...
		if(!max){
			...
			wait_completion(t_info, t_info->queue_count, tid);
		...
		cnt = make_jobs(t_info, ioq, jobq, max, tid);
		if(!cnt && trace_eof(trace)){
			goto Timeout;
		}else{
			t_info->queue_count += cnt;
...
Timeout:
	...
	gettimeofday(&io_stat->end_time, NULL);
	io_stat->execution_time = total_interval;
	//printf(" Finalizing thread %d ... %s\n", (int)tid, trace->tracename);

	pthread_mutex_lock(&t_info->mutex);
	t_info->done = 1;
	//printf("trace_done !\n");
	//printf("Req list size : %d\t%d \t\n", req_list->req_num, req_list->trace_done);
	//req_list->trace_done = 1;
...
int print_result(int nr_trace, int nr_thread, FILE *fp, int detail){
	...
	if(detail){
		...
		if(timeout){
			printf(" req num :%d\ttrace_done :%d\t time = %.0fs (remaining = %.0fs) avgbw = %.3fMB/s curbw = %.3fMB/s Lat = %.6fs, time_diff = %.6fs ",
					req_list->req_num, req_list->trace_done, 
...
int trace_io_put(char* line, struct trace_info_t* trace, int qdepth){
	...
	if(trace->trace_buf_size <= trace->trace_io_cnt){
		...
		trace->trace_buf = (struct trace_io_req*)realloc(trace->trace_buf, sizeof(struct trace_io_req) * trace->trace_buf_size);
	}
AAA:
...
void synthetic_gen(struct trace_info_t *trace){
	...
	trace->trace_buf = (struct trace_io_req*)(malloc(sizeof(struct trace_io_req) * trace->trace_io_cnt));
...
int destroy(pthread_t *threads, int qdepth){
	...
	for(t=0;t<nr_thread;t++){
		...
		for(i=0;i<qdepth;i++){
			...
			th_info[t].th_buf[i] = NULL;
			//th_info[t].th_jobs[i] = NULL;
		}
...
void save_fined_to_file(){	
	Request *next;
	Request *current = req_list->first;
	int save_num = 512;
	int reqnum;
	
	pthread_mutex_lock(&req_list->rl_mutex);
	reqnum = req_list->req_num;
	pthread_mutex_unlock(&req_list->rl_mutex);

	while(reqnum > 0 && save_num > 0 && current != NULL){
		pthread_mutex_lock(&req_list->rl_mutex);
		...
		if(current->operation == 0){
			dprintf(fd_trace, "%ld.%09ld read bus : %d chip : %d block : %d page : %d\n",
					current->timestamp.tv_sec, 
					current->timestamp.tv_nsec,
					current->bus,
					current->chip,
					current->block,
					current->page
		...
		// write
			...
			dprintf(fd_trace, "%ld.%09ld write bus : %d chip : %d block : %d page : %d\n",
					current->timestamp.tv_sec, 
					current->timestamp.tv_nsec,
					current->bus,
					current->chip,
		...
		// erase
			...
			dprintf(fd_trace, "%ld.%09ld erase bus : %d chip : %d block : %d\n",
					current->timestamp.tv_sec, 
					current->timestamp.tv_nsec,
					current->bus,
					current->chip,
...
void* request_saver(void *data){	 	
	
	int tracedone,reqnum;
	Request* last;

	printf("request_saver start!\n");
	//printf("Req list size : %d\t%d \t\n", req_list->req_num, req_list->trace_done);

	do{
		pthread_mutex_lock(&req_list->rl_mutex);
		tracedone = req_list->trace_done;
		reqnum = req_list->req_num;
		last = req_list->last;
		pthread_mutex_unlock(&req_list->rl_mutex);

		if(tracedone == 0 && reqnum > 512) {
			save_fined_to_file();
		} else if(tracedone == 1 && last != NULL) {
			//printf("last Req file i/o\n Req list size : %d\t%d \t%d\t\n", reqnum, tracedone, tracedone);
			save_fined_to_file();
		}
	}while(tracedone == 0 || last != NULL);

	printf("request_saver done!\n");
	//printf("Req list size : %d\t%d \t\n", req_list->req_num, req_list->trace_done);
...
int main(int argc, char **argv){
	...
	for(i=0;i<nr_trace;i++){
		...
		//printf("trace->total_capacity: %zu\n", trace->total_capacity);
		//return 0;
		...
		if(!strcmp(argv[argc_offset+i*EXT_ARG_NUM], "rand")||
			...
			trace->trace_buf = (struct trace_io_req*)malloc(sizeof(struct trace_io_req) * 1024);
...
#define MULTITHREAD_LOADER
	...
	for(t=0;t<nr_thread;t++){
		...
		if(t_info->fd < 0)
			return -1;
		ioctl(t_info->fd, BLKGETSIZE64, &trace->total_capacity);
		
		for(i=0;i<qdepth;i++){
			t_info->th_buf[i] = allocate_aligned_buffer(MAX_BYTES);
			t_info->th_jobs[i] = (io_job*)malloc(sizeof(struct io_job));
		}
		t_info->buf_cur=0;
	...
	module = PAGE_FTL_MODULE;
	device = BLUEDBM_MODULE;
	...
	FILE *warm_up_trace_File = fopen("warm_up_trace.dat", "r");
	if (warm_up_trace_File == NULL) {
		perror("warm up trace file open error\n");
		return 1;
	}

	unsigned char *buffer = (unsigned char *)alloc_buffer((size_t)8192);
	g_assert(buffer != NULL);

	unsigned long long offset;
	size_t bufsz;
	while(fgets(warm_up_line, 200, warm_up_trace_File) != NULL){
		sscanf(warm_up_line, "%lld %zd\n", &offset, &bufsz);
		flash->f_op->write(flash, buffer, (size_t)bufsz, offset * 8192);
	}
	free_buffer(buffer);
	
	fclose(warm_up_trace_File);

	if( 0 < (fd_trace = open("trace_HILS.txt", O_WRONLY | O_CREAT | O_APPEND , 0644))){
		req_list->warm_up_flag = 1;
		printf("warm up is done!\n");
	} else {
		printf("failed to create trace_HILS.txt\n");
	}

	pthread_t rq_saver;

	rc = pthread_create(&rq_saver, NULL, request_saver, (void *)NULL);
	if (rc){
		printf("ERROR; return code from pthread_create( is request saver)\n");
        exit(-1);
	}
	...
	pthread_mutex_unlock(&req_list->rl_mutex);
	printf("trace_done !\n");
	//printf("Req list size : %d\t trace_done: %d\t req_list->last: %p\n", req_list->req_num, req_list->trace_done, req_list->last);
	
	rc = pthread_join(rq_saver, NULL);
	if (rc){
		printf("ERROR; return code from pthread_create( is join req saver\n");
		exit(-1);
	}
